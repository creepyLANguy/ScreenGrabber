...
inline void CheckConfigValues_General()
{
  ...
  WARNIF(physicalStripBrightness, Is::LessThanZero);
  if (physicalStripBrightness > 100) { WARN(physicalStripBrightness, Is::MoreThanMax); }
  ...
  if (mode < Mode::_first) { WARN(deltaEType, Is::NotAnOption); }
  if (mode > Mode::_last) { WARN(deltaEType, Is::NotAnOption); }
