...
void InitialiseBorderChunks(vector<BorderChunk>& borderChunks, const int bitmap_width, const int bitmap_height, const float borderSamplePercentage, const int originPositionOffset, LEDsCollection& leds)
{
  ...
  const int chunk_right_height = neededRightArea / leds.LED_COUNT_RIGHT; //+ (neededRightArea % (neededRightArea / leds.LED_COUNT_RIGHT) > 0 ? 1 : 0);
  ...
  const int chunk_left_height = neededLeftArea / leds.LED_COUNT_LEFT; //+ (neededLeftArea % (neededLeftArea / leds.LED_COUNT_LEFT) > 0 ? 1 : 0);
  ...
  //SetBorderChunkIndexes(borderChunks, originPositionOffset);
  int i = 0;
